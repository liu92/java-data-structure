...
```
1、一个算法的运算时间是指一个算法在计算机上运行所消耗的时间大致可以等于计算机执行一种简单的操作(如赋值、比较、移动等)所需的时间与算法中进行的简单操作次数乘积。
      算法运行时间= 一个简单操作所需的时间 * 简单操作次数
2、也即算法中每条语句的执行时间之和     
      算法运行时间 = 每条语句的执行次数 * 该语句执行一次所需的时间
      
  每条语句执行一次所需的时间,一般是随机器而异的。取决于机器的指令性能、速度以及编译的代码质量。是由机器本身软硬件环境觉定的,它与算法无关。
  所以，我们可以假设执行每条语句所需的时间均为单位时间。次数对算法的运行时间的讨论就可以转化为讨论该算法中所有语句的执行次数,即频度之和了。
  这就可以独立于不同机器的软硬件环境分析算法的时间性能了。
      
```



![image](image/image-20200528161025.png)
...
为了便于比较不同算法的时间效率，我们仅比较它们的数量级   
 ...
 若某个辅助函数f(n),使得当n趋近于无穷大时，T(n)/f(n)的极限值为不等于零的常数，
...
例如:对于求解矩阵相乘问题，算法耗费时间:
 ...
 ```
   ...
   ![image](image/image-20200528161520.png)

 

   ![image](image/image-20200528161618.png)
...
![image](image/image-20200528161959.png)
...
![image](image/image-20200528162147.png)



![image](image/image-20200528162244.png)
...
#### 1.4.4)、算法与算法分析4
  ...
  ```
请注意:有的情况下,算法中基本操作重复执行的次数还随问题输入数据集不同而不同。
  ```

![image](image/image-20200528162729.png)

```
最坏时间复杂度:指在最坏情况下,算法的时间复杂度。
平均时间复杂度: 指在所有可能输入实例在等概率出现的情况下,算法的期望运行时间。
最好时间复杂度: 值在最好情况下,算法的时间复杂度。
   一般总是考虑在最坏情况下的时间复杂度,以保证算法的运行时间不会比它更长。

对于复杂的算法,可以将它分成几个容易估算的部分，然后利用大O加法法则和乘法法则,计算算法的时间复杂度:
a).加法法则
 T(n) =  T1(n)+ T(n) = O(f(n)) + O(g(n)) = O(max(f(n),g(n)))
b).乘法法则
 T(n) =  T1(n) * T(n) = O(f(n)) * O(g(n)) = O(f(n) * g(n))) 
```



![image](image/image-20200528165102.png)
...
![image](image/image-20200528170519.png)
...
![image](image/image-20200528170651.png)



2.1、线性表的定义和特点---表要研究的内容
...
从以上例子可看成线性表的逻辑特征是:
 ...
 ![image](image/image-20200529095956.png)
...
![image](image/image-20200529143414.png)
...
-许多实际应用问题涉及的基本操作有很大相似性,不应为每个具体应用单独编写一个程序。
...
### 2.4.2、线性表的顺序表示和实现2---顺序存储表示

   线性表顺序存储结构的图示:

![image](image/image-20200529101000.png)
...
![image](image/image-20200529101714.png)
...
### 2.4.5、线性表的顺序表示和实现5---插入算法

![image](image/image-20200529103423.png)
...
### 2.4.6、线性表的顺序表示和实现6---删除算法



![image](image/image-20200529103816.png)
...
* 顺序表的删除
  ...
  Status ListDelete_Sq(SqList &L, int i){
     ...
     for(j=i; j<=L.length-1;j++)
      L.elem[j-1]=L.elem[j];  //被删除除元素之后的元素前移
      L.length--;          //表长减1
   return OK;   
  }
  ```
...
![image](image/image-20200529105127.png)
...
​        ![image](image/image-20200510172014.png)
  ...
  (4)、头指针、头结点和首结点 ,头结点是附加的，而这个带头结点的链表中往往不是存储第一个元素的，而是我们额外附加的结点。
    ![image](image/image-20200510172239.png)
    

* 前面的例子中的链表的存储结构示意图有以下两种形式:

  ![image](image/image-20200529111254.png)
...
* 知识回顾
  ...
  ![image](image/image-20200529112056.png)
...
* 带头结点的单链表
  ...
  ![image](image/image-20200529112456.png)
...
struct Lnode *next; 这个表示不是普通的变量，这个指针前面加了* 后，表示指向这个Lode 类型的指针。
 比如：想想让next它指向 a, 那么就将 a地址 而不是a里面的值。来赋值给next; 所以使用& 运算符来取地址
   Lode next = &a;
...
### 2.5.3、线性表的链式表示和实现4--单链表基本操作2--销毁单链表
...
```
### 2.5.4、线性表的链式表示和实现5--单链表基本操作3--清空链表
...
```
### 2.5.5、线性表的链式表示和实现6--单链表基本操作4--求链表的表长
...
### 2.5.6、线性表的链式表示和实现7--单链表基本操作5--取第i个元素

 先进行知识回顾

![image](image/image-20200528173428.png)
...
![image](image/image-20200528173611.png)

### 2.5.7、线性表的链式表示和实现8--单链表基本操作6--按值查找
...
//按值查找 ，返回位置序号
 ...
 ```
### 2.5.8、线性表的链式表示和实现9--单链表基本操作7--插入节点 

![image](image/image-20200528172952.png)
...
示例代码
 ...
 ```
### 2.5.9、线性表的链式表示和实现10--单链表基本操作实现8---删除第i个结点
...
#### (5)、利用二叉树求解表达式的值    ![image](image/image-20200514230057.png)    
...
#### 性质1: 在二叉树的第i层上至多有2<sup>(i-1)</sup>个结点(i>=1) 
  ...
  * 提问:第i层上至少有_个结点? 答:至少有一个结点。
   ...
   * 归纳证明:由归纳假设可知,第i-1层上至多有2<sup>(i-2)</sup>个结点。由于二叉树每个结点的度最大为2，
        ...
        这里说明下：在第i层 是 第i-1上 最大结点树的 2倍，比如这棵树有4层，那么第2层的结点个数是2个，是第1层结点个数是2倍
...
#####   (1)、遍历定义---顺着某一条搜索路径巡防二叉树中的结点,使得每个结点均被访问一次 而且仅被访问一次(又称周游)。
...
#####   (3)、遍历用途---它是树结构插入、删除、修改、查找和排序运算的前提，是二叉树一切运算的基础和核心。    ![image](image/image-20200515151724.png) 
...
```
所谓的遍历，其实就是围绕着 根结点来进行处理，先序遍历就先遍历根结点，中序就是先左节点然后根节点最后右节点
  对于 后续遍历就是先左节点，再右节点 最后根结点 
...
#### 10.6.2、树的存储结构---孩子兄弟表示法(二叉树表示法,二叉链表表示法)
...
的结构复杂，但它的操作有着良好的最坏情况运行时间，
  ...
  * 性质：
  ...
  ```
  * 根据性质 5：红黑树从根节点到每个叶子节点的路径都包含相同数量的黑色节点，
  因此从根节点到叶子节点的路径中包含的黑色节点数被称为树的”黑色高度（black-height）”。
  
  ```
性质 4 则保证了从根节点到叶子节点的最长路径的长度不会超过任何其他路径的两倍。
